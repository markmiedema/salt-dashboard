...
export class ClientService {
  ...
  static async getStats(): Promise<ClientStats> {
    ...
    const recentlyConverted = clients.filter(
      (c) =>
        c.status === 'active' &&
  ...
  static async getTopClientsByRevenue(
    limit: number = 5
  ): Promise<Array<Client & { totalRevenue: number; projectCount: number }>> {
    // This would typically be done with a JOIN query, but we'll simulate it
    const clients = await this.getAll();
    const { data: projects } = await supabase.from('projects').select('client_id, amount');
    ...
    return clientRevenue.sort((a, b) => b.totalRevenue - a.totalRevenue).slice(0, limit);
  }
}
