...
import { TrendingUp, TrendingDown, DollarSign, Calendar, Target, RefreshCw } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, LineChart, Line } from 'recharts';
import { useRevenueStatsQuery, useMonthlyTrendsQuery } from '../../hooks/useReactQuery';

interface ReactQueryRevenueSummaryProps {
  targetMonthly?: number;
}

const ReactQueryRevenueSummary: React.FC<ReactQueryRevenueSummaryProps> = ({ 
  targetMonthly = 75000 
}) => {
  const { 
    data: stats, 
  ...
  const hasError = statsError || trendsError;
  const isStale = statsStale || trendsStale;

  if (isLoading && !stats && !trends) {
    return (
  ...
  const metrics = [
    {
      title: 'Current Month',
      value: safeStats.currentMonth,
      change: safeStats.monthlyGrowth,
      icon: DollarSign,
      color: 'blue',
      target: targetMonthly,
      progress: (safeStats.currentMonth / targetMonthly) * 100
    },
    {
      title: 'Year to Date',
      value: safeStats.yearToDate,
      change: safeStats.yearlyGrowth,
      icon: Calendar,
      color: 'emerald',
      comparison: `vs ${safeStats.lastYear.toLocaleString()} last year`
    },
    {
      title: 'Monthly Average',
      value: safeStats.averageMonthly,
      change: ((safeStats.averageMonthly - safeStats.lastMonth) / safeStats.lastMonth) * 100,
      icon: TrendingUp,
      color: 'purple',
    ...
    },
    {
      ...
      value: safeStats.projectedYearly,
      change: ((safeStats.projectedYearly - safeStats.lastYear) / safeStats.lastYear) * 100,
      icon: Target,
      color: 'orange',
  ...
  const chartData = safeTrends.slice(-12).map((trend, index, array) => ({
    ...trend,
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 pt-6 border-t border-gray-200">
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-900">
              {safeStats.monthlyGrowth > 0 ? '+' : ''}{safeStats.monthlyGrowth.toFixed(1)}%
            </div>
            <div className="text-sm text-gray-600">Monthly Growth</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-900">
              ${(safeStats.projectedYearly / 1000000).toFixed(1)}M
            </div>
            <div className="text-sm text-gray-600">Projected Annual</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-900">
              {safeStats.targetProgress.toFixed(0)}%
            </div>
...
export default ReactQueryRevenueSummary;